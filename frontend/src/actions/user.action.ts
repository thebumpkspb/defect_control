...
export async function login(form: ILoginForm): Promise<ILoginResponse> {
  ...
  const { data } = await axiosInstance.post<ILoginResponse>(`/api/users/login`, body);
  return data;
}
...
export async function registerUesr(form: IUserRegisterForm): Promise<any> {
  ...
  const { data } = await axiosInstance.post<any>(`/api/users/register`, body);
  return data;
}
...
export async function updateUser(form: IUserUpdateForm): Promise<any> {
  ...
  const { data } = await axiosInstance.post<any>(`/api/users/update_user`, body);
  return data;
}
export async function changePassword(form: IChangePasswordForm): Promise<any> {
  ...
  const { data } = await axiosInstance.post<any>(`/api/users/change_password`, body);
  return data;
}

export async function forgotPassword(form: IForgotPasswordForm): Promise<any> {
  const { data } = await axiosInstance.post<any>(`/api/users/reset_password`, {
...
export async function resetPassword(form: IForgotPasswordForm): Promise<any> {
  const { data } = await axiosInstance.post<any>(`/api/users/reset_password_to_default`, {
    credential: form.credential,
  });
  return data;
}
