...
import { inlineOutlineExportAbnormalOccurrenceDownload, inlineOutlineExportDescriptionDownload } from "@/lib/api";
import { InlineOutlineInput } from "@/types/inlineOutline";

interface ExportDescriptionProp {
  // dataSource: CauseOfAbnormalResult;
  input: InlineOutlineInput;
  username: string;
}

const ExportDescription: React.FC<ExportDescriptionProp> = ({input, username}) => {
...
const handleDownloadExportFile = async () => {
  ...
  await inlineOutlineExportDescriptionDownload({"month": input.month, "line": input.line || []});
