...
class ProductionsManager:
    ...
    async def get_prod_qty(
        ...
        if shift.lower() == "all":
            ...
            shift_manual = f"('{shift.upper()}')"
            shift_auto = "(2)"
        start_date_d, end_date_d = get_month_start_end(date=date_time, date_only=True)
        ...
        res_manual = self.crud.get_prod_qty_manual(
            where_stmt=where_stmt_manual, db=db_ms
        )
        ...
        res_auto = self.crud.get_prod_qty_auto(where_stmt=where_stmt_auto, db=db_my)
        return_list = []
