...
def inline_outline_routers(
    ...
    # print("db_common_pg_async:", type(db_common_pg_async))

    @router.get(
        "/default_defect_summary",
        response_model=Default_Defect_Summary_Result_Response,
    ...
    async def default_defect_summary(
        ...
        return Default_Defect_Summary_Result_Response(
            default_defect_summary_result=await inline_outline_manager.get_default_defect_summary(
                db_common_pg_async=db_common_pg_async
            )
        )

    @router.post(
        "/department_section_change",
        response_model=Department_Section_Result_Response,
        dependencies=[Depends(api_key_auth)],
    )
    async def department_section_change(
        text_data: Department_Section,
        ...
        """
        return Department_Section_Result_Response(
            department_section_result=await inline_outline_manager.post_department_change(
    ...
    @router.post(
        "/defect_summary",
        response_model=Defect_Summary_Result_Response,
        dependencies=[Depends(api_key_auth)],
    )
    async def defect_summary(
        text_data: Defect_Summary,
        db: AsyncSession = Depends(db),
        db_common_pg_async: AsyncSession = Depends(db_common_pg_async),
        ...
        """
        return Defect_Summary_Result_Response(
            defect_summary_result=await inline_outline_manager.post_defect_summary(
                text_data=text_data,
                db=db,
                db_common_pg_async=db_common_pg_async,
    ...
    @router.post(
        "/cause_of_abnormal",
        response_model=Cause_Of_Abnormal_Result_Response,
        dependencies=[Depends(api_key_auth)],
    )
    async def cause_of_abnormal(
        text_data: Defect_Summary_cause,
        db: AsyncSession = Depends(db),
        db_common_pg_async: AsyncSession = Depends(db_common_pg_async),
    ):
        """
        ...
        """
        return Cause_Of_Abnormal_Result_Response(
            cause_of_abnormal_result=await inline_outline_manager.post_cause_of_abnormal(
                text_data=text_data,
                db=db,
                db_common_pg_async=db_common_pg_async,
            )
    ...
    @router.post(
        "/defect_pareto_chart",
        response_model=Defect_Pareto_Chart_Result_Response,
        dependencies=[Depends(api_key_auth)],
    )
    async def defect_pareto_chart(
        ...
        \nexp2: { "month": "November-2024", "department": "Manufacturing", "section": "414454 - Sta. Assy PA70", "line": ["414454 - Mag. Sw. PA", "414454 - Starter Assy PA70"] }
        """
        return Defect_Pareto_Chart_Result_Response(
            defect_pareto_chart_result=await inline_outline_manager.post_defect_pareto_chart(
                text_data=text_data,
                db=db,
                db_common_pg_async=db_common_pg_async,
                db_prod_ms=db_prod_ms,
    ...
    @router.post(
        "/export_abnormal_occurrence",
        dependencies=[Depends(api_key_auth)],
    )
    async def export_abnormal_occurrence(
        text_data: Export_Abnormal,
        db: AsyncSession = Depends(db),
        db_common_pg_async: AsyncSession = Depends(db_common_pg_async),
    ):
        """
        ...
        """
        return await inline_outline_manager.post_export_abnormal_occurrence(
    ...
    async def export_description(
        ...
        """
        return await inline_outline_manager.post_export_description(
            text_data=text_data,
            db=db,
            db_common_pg_async=db_common_pg_async,
            db_prod_ms=db_prod_ms,
